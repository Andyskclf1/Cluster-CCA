      ...
      if (sum(cluster_index == partition_update) < min_cluster_size)
      {
        ...
        for (affected_cluster_index in c(worst_cluster, cluster_index))
        {
          ...
          if (sum(affected_cluster_index == partition_update) >= min_cluster_size)
          {
            ...
            cluster_subject_indices = which(partition_update == affected_cluster_index)
            
            # scale cluster and update data? think about this
            
            # reduce matrices
            affected_cluster = lapply( data , function(X){
              X[cluster_subject_indices,] } ) 
            
            # recompute parameters
            pars = compute_single_CCA( affected_cluster , component_count )
            
            # retrieve subject-specific losses for cluster
            min_subject_losses[cluster_subject_indices] = pars$loss$`loss per subject` 
          }
        }
      }
    }
