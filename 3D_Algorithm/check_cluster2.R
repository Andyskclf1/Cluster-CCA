                               ...
                               component_count , subject_lossAll_init )
{
  ...
  LossPerObjVector = numeric( nClust ) # subject loss per cluster
  
  #global loss for solution when subject is reassigned to each cluster
  LossSolReassigned = numeric( nClust )
  
  #loss for cluster in which subjectNr is removed
  cluster_subjects_index = which( partition==partition[subjectNr] )
  cluster_subjects_index_Minussubject = cluster_subjects_index[ cluster_subjects_index != subjectNr ]
  tempout = compute_single_CCA( data, cluster_subjects_index_Minussubject , component_count )
  ...
  cluster_subjects_index_all = NULL 
  ...
  for( clust in 1:nClust )
  {
    ...
    else
    {
      ...
      cluster_subjects_index_all[[clust]] = sort( c( which( partition==clust ) , subjectNr ) )
      tempout = compute_single_CCA( data, cluster_subjects_index_all[[clust]], component_count )
      templossPerClust[[clust]][clust] = tempout$loss$`cluster loss`
      LossPerObjVector[clust] =
        tempout$loss$`loss per subject`[cluster_subjects_index_all[[clust]]==subjectNr]
  ...
  {
    #changes loss for subjects from cluster to which subject under consideration belonged
    subject_lossAll_update[ cluster_subjects_index_Minussubject ] = 
      subject_lossMinussubject
    
    #changes loss for subjects from cluster to which subject under consideration is reallocated
    subject_lossAll_update[ cluster_subjects_index_all[[opt_clust]] ] =
