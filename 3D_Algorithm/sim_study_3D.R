# extensive simulation study to evaluate performance of 3D algorithm under
...
pars_list = list("subject_count" = c(50, 100, 150),
                 "variable_b_count" = c(5, 10, 15), 
...
for (data_rep in 1:data_rep_count){
  for (pars_index in 1:nrow(pars_grid)){
    data = sim_data(subject_count = pars_grid$subject_count[pars_index], 
                    variable_b_count = pars_grid$variable_b_count[pars_index], 
